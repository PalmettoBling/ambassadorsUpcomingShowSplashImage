...
function drawImage() {
    ...
    if (canvas.getContext) {
        ...
        img.onload = function() {
            ...
            ctx.fillText(schedule[0].gameName, x, y); // x, y

            boxArtURL = schedule[0].gameImage.replace("{width}x{height}", "200x300")
            boxart.src = boxArtURL;
            ctx.drawImage(boxart, (x-100), (y+10)); // x-100, y+10

            ctx.font = "bold 20px 'Segoe UI'"
            ctx.fillText(schedule[0].showStartTime, x, (y + 340)) // x, y+340

            ctx.font = "bold 25px 'Segoe UI'"
            ctx.fillText(schedule[0].showName, x, (y+370)) //x, y+370

            //show 2
            ctx.font = "bolder italic 34px 'Segoe UI',Arial,sans-serif";
            ctx.fillText(schedule[1].ganeName, (2 * x), y); // 2x, y

            boxArtURL = schedule[1].gameImage.replace("{width}x{height}", "200x300")
            boxart.src = boxArtURL;
            ctx.drawImage(boxart, ((2* x)-100), (y+10)); // 2x-100, y+10
            
            ctx.font = "bold 20px 'Segoe UI'"
            ctx.fillText(schedule[1].showStartTime, (2* x), (y + 340)) // 2x, y+340
            
            ctx.font = "bold 25px 'Segoe UI'"
            ctx.fillText(schedule[1].showName, (2* x), (y+370)) //2x, y+370

            //show 3
            ctx.font = "bolder italic 34px 'Segoe UI',Arial,sans-serif";
            ctx.fillText(schedule[2].gameName, (3* x), y); // 3x, y

            boxArtURL = schedule[2].gameImage.replace("{width}x{height}", "200x300")
            boxart.src = boxArtURL;
            ctx.drawImage(boxart, ((3* x)-100), (y+10)); // 3x-100, y+10

            ctx.font = "bold 20px 'Segoe UI'"
            ctx.fillText(schedule[2].showStartTime, (3* x), (y + 340)) // 3x, y+340
            
            ctx.font = "bold 25px 'Segoe UI'"
            ctx.fillText(schedule[1].showName, (3* x), (y+370)) //3x, y+370
            //end of show cards
